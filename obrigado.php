...
$payment_id = $_GET['payment_id'] ?? $_GET['payment_intent'] ?? null;
$sale_details = null;
$tracking_config = [];
$fb_events_enabled = [];
$gg_events_enabled = [];
        ...
        if ($sale_details && $sale_details['email_entrega_enviado'] == 0) {
            ...
            if ($sale_details['status_pagamento'] === 'approved') {
                ...
                if ($gateway_info) {
                    ...
                    // Verifica Stripe
                    elseif (!empty($gateway_info['stripe_secret_key']) && ($sale_details['metodo_pagamento'] === 'credit_card_stripe' || $sale_details['metodo_pagamento'] === 'stripe' || $sale_details['metodo_pagamento'] === 'Stripe Cartão')) {
                        // Para Stripe, o transacao_id é o PaymentIntent ID
                        $ch = curl_init('https://api.stripe.com/v1/payment_intents/' . $payment_id);
                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                        curl_setopt($ch, CURLOPT_USERPWD, $gateway_info['stripe_secret_key'] . ':');
                        curl_setopt($ch, CURLOPT_TIMEOUT, 10);
                        $stripe_res = json_decode(curl_exec($ch), true);
                        $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                        curl_close($ch);

                        if ($http_code == 200 && isset($stripe_res['status'])) {
                            $status_real = $stripe_res['status'];
                            if ($status_real === 'succeeded') {
                                $status_aprovado = true;
                                // Atualiza status no BD
                                $pdo->prepare("UPDATE vendas SET status_pagamento = 'approved' WHERE transacao_id = ?")->execute([$payment_id]);
                                error_log("Obrigado.php: Status atualizado para 'approved' no BD após verificação na API Stripe");
                            }
                        }
                    }
                }
    ...
    <?php if ($sale_details): ?>
        ...
        <?php if (!empty($fbPixelId) && !empty($fb_events_enabled['purchase'])): ?>
            ...
            <noscript><img height="1" width="1" style="display:none"
                    src="https://www.facebook.com/tr?id=<?php echo htmlspecialchars($fbPixelId); ?>&ev=Purchase&cd[value]=<?php echo (float) $sale_details['valor']; ?>&cd[currency]=BRL" /></noscript>
        <?php endif; ?>
        ...
        <?php if ((!empty($gaId) || !empty($gAdsId)) && !empty($gg_events_enabled['purchase'])):
            ...
            ?>
            <script async
                src="https://www.googletagmanager.com/gtag/js?id=<?php echo htmlspecialchars($google_primary_id); ?>"></script>
            <script>
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-lg p-6 sm:p-8 bg-white rounded-2xl shadow-lg text-center">
        ...
        <p class="text-gray-500 mt-3 text-sm">
            <strong>Importante:</strong> Caso não receba o e-mail em alguns minutos, verifique também sua <strong>caixa
                de spam</strong> ou lixo eletrônico.
        </p>
