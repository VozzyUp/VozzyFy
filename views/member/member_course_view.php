    ...
    try {
        ...
        if (!$acesso_info) {
            ...
            if (!$curso) {
                ...
                if (!isset($curso['allow_comments']))
                    $curso['allow_comments'] = 0;
                if (!isset($curso['community_enabled']))
                    $curso['community_enabled'] = 0;
                ...
                foreach ($modulos as $modulo) {
                    if (!isset($modulo['secao_id']))
                        $modulo['secao_id'] = null;
                            ...
                            foreach ($sp_rows as $sp) {
                                ...
                                if ($prod) {
                                    ...
                                    if (empty($prod['checkout_hash'])) {
                                        ...
                                        if ($oferta)
                                            $prod['checkout_hash'] = $oferta['checkout_hash'];
                                    }
...
<head>
    ...
    <title><?php echo htmlspecialchars($curso['produto_nome'] ?? $curso['titulo'] ?? 'Curso'); ?> - Área de Membros
    </title>
    <?php
    ...
    <style>
        ...
        .prose {
            /* TailwindCSS Typography plugin classes can be added here or in global CSS */
        ...
        #player-wrapper .player-card .border-gray-700 {
            border-color: #262626 !important;
        }

        #player-wrapper .player-aside {
            background-color: #141414;
            border: 1px solid #262626;
        }

        #player-wrapper .lesson-item:hover {
        ...
        #player-wrapper input.bg-gray-700,
        #player-wrapper textarea.bg-gray-700 {
            background-color: #1a1a1a !important;
        ...
        .lesson-item.active .lucide-play-circle {
            color: #fdba74;
        }
        ...
        .lesson-item.locked {
            ...
            /* Mais escuro para indicar bloqueio */
        }
        ...
        .lesson-item.locked .lucide-play-circle,
        .lesson-item.locked .lucide-lock,
        .lesson-item.locked .lucide-file-text {
            color: #718096;
            /* Cinza para ícones bloqueados */
        }
        ...
        .ymin .icon {
            ...
            /* <-- FORÇA O ÍCONE GRANDE DE PLAY A SER BRANCO */
        }
        ...
        .ymin .timecode.ui {
            ...
            color: #fff;
            font: 600 12px/1.2 system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, "Helvetica Neue", Arial, "Noto Sans", "Apple Color Emoji", "Segoe UI Emoji";
            /* <-- COR DO TEXTO (BRANCO) */
        }
        ...
        .ymin .btn {
            ...
            color: #fff;
            /* <-- COR DO BOTÃO (LARANJA) E ÍCONE (BRANCO) */
            display: grid;
            place-items: center;
            cursor: pointer;
        ...
        .ymin .btn img {
            ...
            pointer-events: none;
            filter: brightness(0) invert(1);
            /* <-- FORÇA OS ÍCONES DOS BOTÕES A SEREM BRANCOS */
        }

        :fullscreen .ymin .frame {
            aspect-ratio: auto;
            height: 100vh
        }

        :-webkit-full-screen .ymin .frame {
            aspect-ratio: auto;
            height: 100vh
        }
        ...
        .ymin .speed-menu.show,
        .ymin .quality-menu.show {
            display: block;
        }

        .ymin .speed-menu button,
        ...
        .ymin .speed-menu button:hover,
        .ymin .quality-menu button:hover {
            background: rgba(255, 255, 255, 0.1);
        }

        .ymin .speed-menu button.active,
        .ymin .quality-menu button.active {
            background: var(--accent);
            color: #fff;
        }

        .ymin .speed-menu button.active::after,
...
<body class="text-gray-200 antialiased" style="background-color: #0d0d0d;">
        ...
        <div class="flex min-h-screen items-center justify-center p-8">
            <div class="bg-red-900 border border-red-700 text-red-200 px-6 py-4 rounded-lg text-center max-w-lg">
                ...
                <a href="/member_area_dashboard"
                    class="mt-4 inline-block bg-red-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-700 transition">Voltar
                    aos Meus Cursos</a>
            </div>
        </div>
    <?php elseif (!$curso): ?>
    ...
    <?php else:
        ...
        <div id="course-container" class="min-h-screen flex flex-col">
            <?php if ($aba_comunidade): ?>
                <header class="w-full flex-shrink-0 text-white p-4 md:p-6 border-b border-gray-700/50"
                    ...
                    <div class="max-w-screen-2xl mx-auto flex justify-between items-center flex-wrap gap-3">
                        <div class="flex items-center space-x-4">
                            ...
                            <h1 class="text-xl md:text-2xl font-bold">
                                <?php echo htmlspecialchars($curso['produto_nome'] ?? $curso['titulo'] ?? 'Comunidade'); ?>
                            </h1>
                        </div>
                        <div class="flex items-center gap-3 md:gap-4">
                            <?php if ($community_enabled): ?>
                                <a href="/member_course_view?produto_id=<?php echo (int) $produto_id; ?>"
                                    class="px-3 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 transition text-sm font-medium">Member</a>
                                <a href="/member_course_view?produto_id=<?php echo (int) $produto_id; ?>&aba=comunidade"
                                    class="px-3 py-2 rounded-lg bg-white/10 text-white text-sm font-medium">Comunidade</a>
                            <?php endif; ?>
                            <span class="font-medium hidden md:block text-gray-300">Olá,
                                <?php echo htmlspecialchars($cliente_nome); ?>!</span>
                            <a href="/member_logout"
                                class="flex items-center space-x-2 text-white/90 hover:text-white transition-colors px-3 py-2 rounded-lg hover:bg-white/10">
                    ...
                    <main class="max-w-screen-2xl mx-auto p-4 md:p-8 w-full flex-1">
                        ...
                        <?php if (!$mostrar_conteudo): ?>
                            <div class="bg-gray-800 border border-gray-700 p-8 rounded-lg text-center text-gray-400">
                                ...
                                <p class="mt-4 font-semibold text-lg text-gray-200">Este curso ainda não tem conteúdo disponível.
                                </p>
                            ...
                            <div id="player-wrapper" class="hidden">
                                ...
                                <div class="mb-8">
                                    <div class="flex justify-between items-center mb-2">
                                        <span class="text-sm font-semibold" style="color: var(--accent-primary);">SEU
                                            PROGRESSO</span>
                                        <span class="text-sm font-bold text-white"><?php echo $progresso_percentual; ?>%
                                            Completo</span>
                                    </div>
                                    <div class="w-full bg-gray-700 rounded-full h-2.5">
                                        <div class="h-2.5 rounded-full"
                                            style="width: <?php echo $progresso_percentual; ?>%; background-color: var(--accent-primary);">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div id="player-section" class="flex flex-col lg:flex-row gap-8 mb-12">
                                    ...
                                    <div class="lg:w-2/3 w-full">
                                        ...
                                        <div id="player-host" class="bg-black rounded-xl shadow-2xl mb-6">
                                            ...
                                            <div
                                                class="w-full aspect-video bg-black flex flex-col items-center justify-center text-gray-500 rounded-xl">
                                        ...
                                        <div class="player-card bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
                                            <h2 id="lesson-title" class="text-2xl font-bold text-white mb-4">Selecione um módulo
                                                para começar</h2>
                                            ...
                                            <div class="mt-6 pt-4 border-t border-gray-700 flex justify-end">
                                                ...
                                                <button id="mark-as-complete-btn"
                                                    class="text-white font-bold py-2.5 px-5 rounded-lg transition duration-300 flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed hidden">
                                            ...
                                            <?php if (!empty($curso['allow_comments'])): ?>
                                                <div id="comments-section" class="mt-8 pt-6 border-t border-gray-700 hidden">
                                                    ...
                                                    <form id="comment-form" class="mb-6">
                                                        ...
                                                        <input type="hidden" id="comment-author" name="autor_nome"
                                                            value="<?php echo htmlspecialchars($cliente_nome ?? $cliente_email ?? ''); ?>">
                                                        <div class="mb-3">
                                                            <label for="comment-text"
                                                                class="block text-gray-300 text-sm font-medium mb-1">Comentário</label>
                                                            <textarea id="comment-text" name="texto" rows="3"
                                                                class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-offset-0 resize-none"
                                                                style="--tw-ring-color: var(--accent-primary);" maxlength="5000"
                                                                required placeholder="Escreva seu comentário..."></textarea>
                                                        </div>
                                                        <button type="submit" id="comment-submit"
                                                            class="text-white font-bold py-2 px-5 rounded-lg transition"
                                                            style="background-color: var(--accent-primary);">Enviar comentário</button>
                                                    </form>
                                    ...
                                    <aside
                                        class="player-aside lg:w-1/3 w-full bg-gray-800 rounded-xl shadow-lg p-4 flex-shrink-0 h-fit lg:sticky top-20 border border-gray-700">
                                        <h3 id="module-title-aside" class="font-bold text-xl text-white mb-4 px-2">Aulas do Módulo
                                        </h3>
                                        ...
                                        <?php if ($secao['tipo_secao'] === 'curso' && !empty($bloco['modulos_com_aulas'])): ?>
                                            <div class="modules-carousel-section mb-10">
                                                <div class="flex justify-between items-center mb-4">
                                                    <h2 class="text-2xl font-bold text-white"><?php echo htmlspecialchars($secao['titulo']); ?>
                                                    </h2>
                                                    <div class="flex gap-2">
                                                        <button type="button"
                                                            class="carousel-prev w-10 h-10 md:w-10 md:h-10 rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white flex items-center justify-center transition-colors"
                                                            aria-label="Anterior"><i data-lucide="chevron-left" class="w-5 h-5"></i></button>
                                                        <button type="button"
                                                            class="carousel-next w-10 h-10 md:w-10 md:h-10 rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white flex items-center justify-center transition-colors"
                                                            aria-label="Próximo"><i data-lucide="chevron-right" class="w-5 h-5"></i></button>
                                                    </div>
                                                </div>
                                                <div class="modules-carousel-wrapper">
                                                    <div class="modules-carousel-track flex gap-6 overflow-x-auto overflow-y-visible scroll-smooth pb-2"
                                    ...
                                    <?php if (!empty($modulos_sem_secao)): ?>
                                        <div id="modules-grid" class="modules-carousel-section mb-10">
                                            <div class="flex justify-between items-center mb-4">
                                                ...
                                                <div class="flex gap-2">
                                                    <button type="button"
                                                        class="carousel-prev w-10 h-10 rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white flex items-center justify-center transition-colors"
                                                        aria-label="Anterior"><i data-lucide="chevron-left" class="w-5 h-5"></i></button>
                                                    <button type="button"
                                                        class="carousel-next w-10 h-10 rounded-lg bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white flex items-center justify-center transition-colors"
                                                        aria-label="Próximo"><i data-lucide="chevron-right" class="w-5 h-5"></i></button>
                                                </div>
                                            </div>
                                            <div class="modules-carousel-wrapper">
                ...
                <?php else: ?>
                    ...
                    if ($comunidade_banner && file_exists($comunidade_banner)):
                        ...
                        <div class="w-full mb-6">
                            <img src="<?php echo htmlspecialchars($comunidade_banner); ?>" alt="Banner Comunidade"
                    ...
                    <main class="max-w-screen-2xl mx-auto p-3 md:p-5 w-full flex-1 flex gap-6" id="community-main">
                        <?php if (empty($comunidade_categorias)): ?>
                            <div class="w-full p-8 rounded-lg text-center text-gray-400"
                        ...
                        <?php else: ?>
                            ...
                            <aside class="w-64 flex-shrink-0 hidden md:block">
                                <div class="rounded-lg border p-4 sticky top-20"
                                    style="background-color: #141414; border-color: #262626;">
                                    <h3 class="text-lg font-bold text-white mb-4">Categorias</h3>
                                    <nav class="space-y-2">
                                        <?php foreach ($comunidade_categorias as $idx => $cat): ?>
                                            <button type="button"
                                                class="community-tab w-full text-left px-3 py-2 rounded-lg border transition text-gray-400 hover:text-white <?php echo $idx === 0 ? 'text-white' : ''; ?>"
                                                style="<?php echo $idx === 0 ? 'background-color: #1a1a1a; border-color: #333;' : 'border-color: #262626; background-color: transparent;'; ?>"
                                                onmouseover="<?php echo $idx === 0 ? '' : 'this.style.backgroundColor=\'#1a1a1a\'; this.style.borderColor=\'#333\';'; ?>"
                                                onmouseout="<?php echo $idx === 0 ? '' : 'this.style.backgroundColor=\'transparent\'; this.style.borderColor=\'#262626\';'; ?>"
                                                data-categoria-id="<?php echo (int) $cat['id']; ?>"
                                                data-public-posting="<?php echo (int) $cat['is_public_posting']; ?>">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="md:hidden mb-4 overflow-x-auto">
                                    <div class="flex gap-2 pb-2">
                                        <?php foreach ($comunidade_categorias as $idx => $cat): ?>
                                            <button type="button"
                                                class="community-tab flex-shrink-0 px-4 py-2 rounded-lg border transition text-gray-400 hover:text-white <?php echo $idx === 0 ? 'text-white' : ''; ?>"
                                                style="<?php echo $idx === 0 ? 'background-color: #1a1a1a; border-color: #333;' : 'border-color: #262626; background-color: transparent;'; ?>"
                                                onmouseover="<?php echo $idx === 0 ? '' : 'this.style.backgroundColor=\'#1a1a1a\'; this.style.borderColor=\'#333\';'; ?>"
                                                onmouseout="<?php echo $idx === 0 ? '' : 'this.style.backgroundColor=\'transparent\'; this.style.borderColor=\'#262626\';'; ?>"
                                                data-categoria-id="<?php echo (int) $cat['id']; ?>"
                                        ...
                                        <form id="community-post-form-el" class="space-y-4">
                                            ...
                                            <div>
                                                <label for="community-post-content"
                                                    class="block text-gray-300 text-sm font-medium mb-2">Novo post</label>
                                                <textarea id="community-post-content" name="conteudo" rows="4"
                                                    class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white resize-none focus:ring-2 focus:ring-offset-0"
                                                    style="--tw-ring-color: var(--accent-primary);" maxlength="10000" required
                                                    placeholder="Escreva sua mensagem..."></textarea>
                                            </div>
                                            <div>
                                                <label for="community-post-image"
                                                    class="block text-gray-300 text-sm font-medium mb-2">Imagem (opcional)</label>
                                                <input type="file" id="community-post-image" name="imagem" accept="image/*"
                                                    class="w-full text-sm text-gray-400 file:mr-2 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:cursor-pointer file:bg-gray-700 file:text-white">
                                                <div id="community-post-image-preview" class="mt-2 hidden">
                                                    <img id="community-post-image-preview-img" src="" alt="Preview"
                                                        class="max-w-full max-h-64 rounded-lg border border-gray-600">
                                                    <button type="button" id="community-post-image-remove"
                                                        class="mt-2 text-sm text-red-400 hover:text-red-300">Remover imagem</button>
                                                </div>
                                            </div>
                                            <button type="submit" class="text-white font-bold py-2 px-5 rounded-lg transition"
                                                style="background-color: var(--accent-primary);">Publicar</button>
                                        </form>
                                    </div>
                                </div>
                ...
                events: {
                    onReady() {
                        try {
                            ...
                            // yminPlayer.mute(); // REMOVIDO: Não mutar automaticamente
                            // yminPlayer.playVideo(); // REMOVIDO: Não iniciar automaticamente
                        } catch (err) {
                            root.classList.remove('loading');
                        }
    ...
    <div id="download-consent-modal"
        class="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4 hidden">
        <div
            class="bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-700">
            ...
            <form id="consent-form" class="p-6 space-y-4">
                ...
                <div class="flex items-start space-x-3">
                    <input type="checkbox" id="consent-modal-checkbox" name="termos_aceitos" required
                        class="mt-1 h-4 w-4 border-gray-600 rounded"
                        style="color: var(--accent-primary); --tw-ring-color: var(--accent-primary);">
                ...
                <div class="space-y-4 pt-4 border-t border-gray-700">
                    <div>
                        <label for="consent-modal-email"
                            class="block text-gray-300 text-sm font-semibold mb-2">Email</label>
                        <input type="email" id="consent-modal-email" name="email" required readonly
                            class="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white cursor-not-allowed"
                            value="<?php echo htmlspecialchars($cliente_email); ?>">
                        <p class="mt-1 text-xs text-gray-400">Email preenchido automaticamente</p>
                    </div>

                    <div>
                        <label for="consent-modal-cpf"
                            class="block text-gray-300 text-sm font-semibold mb-2">CPF</label>
                        <input type="text" id="consent-modal-cpf" name="cpf" required maxlength="14"
                            placeholder="000.000.000-00"
                            class="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2"
                    ...
                    <div>
                        <label for="consent-modal-nome" class="block text-gray-300 text-sm font-semibold mb-2">Nome
                            Completo</label>
                        <input type="text" id="consent-modal-nome" name="nome" required minlength="3"
                ...
                <div class="flex justify-end space-x-4 pt-4 border-t border-gray-700">
                    <button type="button" id="consent-modal-cancel"
                        class="px-6 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition">
                        Cancelar
                    </button>
                    <button type="submit" id="consent-modal-submit" class="px-6 py-2 text-white rounded-lg transition"
