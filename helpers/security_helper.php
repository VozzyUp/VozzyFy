...
if (!function_exists('validate_webhook_signature')) {
     ...
     * @return bool True se válido, False caso contrário
     */
    function validate_webhook_signature($gateway, $headers, $payload, $secret_key, $raw_payload = null)
    {
        if (empty($secret_key)) {
        ...
        switch (strtolower($gateway)) {
            ...
            case 'stripe':
                $sig_header = $headers['Stripe-Signature'] ?? $headers['HTTP_STRIPE_SIGNATURE'] ?? '';
                if (empty($sig_header) || empty($raw_payload))
                    return false;
                ...
                // Prevent replay attacks (optional tolerance, e.g., 5 minutes)
            ...
            case 'pushinpay':
                ...
                if (!empty($x_signature)) {
                    // Se raw_payload disponível, use-o (mais seguro/correto)
                    $payload_string = $raw_payload ?? json_encode($payload);
